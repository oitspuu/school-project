...
let work = ref<IWorkHours>()

const loadData = async () => {
  if (authStore.jwtInfo) {
    let response = await WorkHoursService.getWorkhour(authStore.jwtInfo, props.id)
    if (response.data) {
    ...
    response = await WorkHoursService.getWorkhour(refresh.data, props.id)
    if (response.data) {
      work.value = response.data
      isLoading.value = false
      return
...
const deleteWork = async () => {
  ...
  let response = await WorkHoursService.deleteWork(authStore.jwtInfo, props.id)
  ...
  if (response.status == 401) {
    ...
    authStore.jwtInfo = refresh.data
    response = await WorkHoursService.deleteWork(authStore.jwtInfo, props.id)
    if (response.status == 204) {
      router.push('/work')
      return
    }
...
<template>
  ...
  <div v-else>
    ...
    <div>
      ...
      <router-link :to="{ name: 'workdetails', params: { id: work?.userWorkId } }"
        >Back to hours list</router-link
      >
    </div>
  </div>
</template>
